...
from utils.HSICommonUtils import normlize3D, ImageStretching, applyPCA
...
from model.DNRGNet import DNRGNet
...
def get_parser():
    ...
    parser.add_argument('--val_ratio', type=float, default=0.99)
    parser.add_argument('--exp_name', type=str, default='RUNS')
    parser.add_argument('--numComponents', type=int, default=3)
...
if __name__ == '__main__':
    ...
    img = applyPCA(data, numComponents)
    ...
    indel = np.argsort(list(Counter(gt.reshape(-1)).keys()))
    we2 = torch.tensor(np.array(list(Counter(gt.reshape(-1)).values()))[indel][1:] + 1, dtype=torch.float32,
                       device=device)
    we2 = 1. / torch.sqrt(we2)
    loss_func = torch.nn.CrossEntropyLoss(ignore_index=-1,weight=we2)
    ...
    for exp_idx,curr_seed in enumerate(seed_list):
        ...
        net = DNRGNet(in_channels=channels,
                      out_channels=32,
        ...
        best_net = DNRGNet(in_channels=channels,
                      out_channels=32,
                      image_size=(height, width),
                      window_size=7,
                      layers=2,
